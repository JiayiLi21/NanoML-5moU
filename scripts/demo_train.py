...
file = open('/home/jiayi/5moU/data/DL_fromTombo/eval_xgb_perf.csv','w',encoding = 'UTF-8')
csv_writer = csv.writer(file)
csv_writer.writerow(['motif','auc','Acc','Precision','Recall','F1'])
for motif in motifs:
    try:
        eval,model,scaler = train_model(motif,data)
        joblib.dump(model, '/home/jiayi/5moU/Code/DL5mou/DeepRead/eval_xgb_model/' + motif + '.joblib')
        joblib.dump(scaler,'/home/jiayi/5moU/Code/DL5mou/DeepRead/eval_xgb_scaler/' + motif + '.joblib')
        print([motif,eval])
