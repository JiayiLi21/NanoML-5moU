...
def train_model(motif,data):
    ...
    F1= metrics.f1_score(y_test,clf.predict(X_test))
    eval=[auc,Acc,Precision,Recall,F1]
    return(eval,clf,scaler)
...
file = open('/home/jiayi/5moU/data/DL_fromTombo/eval_SVM_perf.csv','w',encoding = 'UTF-8')
csv_writer = csv.writer(file)
csv_writer.writerow(['motif','auc','Acc','Precision','Recall','F1'])
for motif in motifs:
    try:
        eval,model,scaler = train_model(motif,data)
        joblib.dump(model, '/home/jiayi/5moU/Code/DL5mou/DeepRead/eval_SVM_model/' + motif + '.joblib')
        joblib.dump(scaler,'/home/jiayi/5moU/Code/DL5mou/DeepRead/eval_SVM_scaler/' + motif + '.joblib')
        print([motif,eval])
