...
def search_motif(signal, start, length, base, fn_string):
    ...
    for indx in range(len(length)):
        if 2 + clipnum <= indx <= len(length) - 3 - clipnum:
            ...
            if len(list_have) == 0:
                continue
            raw_signal_every = [raw_signal[start[indx + x]:start[indx + x] + length[indx + x]] for x in
                                [-2, -1, 0, 1, 2]]
            ...
            """
            line += "%s,%s,%s,%s,%s,%s,%s\n" % (str(fn_string).split("/")[-1], indx, kmer_now_t, ",".join([str(x) for x in mean]),
            ",".join([str(x) for x in std]), ",".join([str(x) for x in md_intense]),
            ",".join([str(x) for x in length2]))
            """

            line += "%s|%s|%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" % (
            str(fn_string).split("/")[-1], indx, "N", base2, kmer_now_t, "|".join([str(x) for x in mean]),
            "|".join([str(x) for x in std]), "|".join([str(x) for x in md_intense]),
            "|".join([str(x) for x in length2]), kmer_now_t)
...
def extract_file(input_file):
    try:
        (raw_data, raw_label, raw_start, raw_length) = get_label_raw(input_file, FLAGS.basecall_group,
    ...
    total_seq = ">%s\n%s\n" % (ids, total_seq)
    line = search_motif(raw_data, raw_start, raw_length, raw_label, input_file)
    print(line)
...
def subcon(fls):
    ...
    #############################
        ...
        for fl in fls:
            ...
            if num and seq:
                nums.append([num, seq])
    output = open(FLAGS.output + ".feature.csv", "w")
...
if __name__ == "__main__":
    ...
    subcon(total_fl)
##################################################################################
#python extract_features_U.py --cpu=20 --fl=/data/jiayi/IVT_tombo/normalU_tombo/fast5_fl_U.txt -o tombo_U_features --clip=10