...
def search_RRACH(signal,start,length,base,fn_string):
	...
	kmer_fillter="[ACGT][ACGT]T[ACGT][ACGT]"
	...
	for indx in range(len(length)):
		if 2+clipnum<=indx<=len(length)-3-clipnum:
			...
			line += "%s,%s,%s,%s,%s,%s,%s\n" % (str(fn_string).split("/")[-1], indx, kmer_now_t, ",".join([str(x) for x in mean]),",".join([str(x) for x in std]), ",".join([str(x) for x in md_intense]),",".join([str(x) for x in length2]))
